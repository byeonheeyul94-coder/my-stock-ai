...
# [핵심 수정] 해외 서버 차단을 피하기 위해 NAVER 모드로 리스트를 가져옵니다.
@st.cache_data(ttl=3600)
def get_krx_list():
    try:
        # 'KRX' 대신 'NAVER'를 사용하면 Streamlit Cloud에서 에러가 안 납니다!
        df = fdr.StockListing('NAVER')
        return df[['Code', 'Name']]
    except:
        # 비상용: 서버가 정 안될 때를 대비한 샘플 데이터
        ...
        if st.button("장부 저장"):
            if in_n in stock_list['Name'].values:
                if in_n in df_pf['종목명'].values: 
                    ...
                    new_row = pd.DataFrame({"종목명":[in_n], "매수가":[in_p], "수량":[in_c]})
...
# --- [메뉴 4: 🎯 템플턴 포착] ---
    ...
    if st.button("🔎 스캔 시작"):
        ...
        for i, (_, row) in enumerate(targets.iterrows()):
            p = (i + 1) / total
            p_bar.progress(p)
            status.markdown(f"**진행률: {p*100:.1f}%** ({i+1} / {total})")
            try:
...
# --- [기타 메뉴 유지] ---
    ...
    if st.button("등록"):
        if new_f in stock_list['Name'].values:
            new_row = pd.DataFrame({"종목명":[new_f]})
            fav_df = pd.concat([fav_df, new_row], ignore_index=True); save_data(fav_df, FAV_FILE); st.rerun()
    if not fav_df.empty:
