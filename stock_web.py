...
def get_stock_list():
    try:
        # 'KRX'는 해외 서버에서 차단될 확률이 매우 높으므로 'NAVER'를 사용합니다.
...
def get_strategy_signal(df, curr_p, buy_p=None):
    ...
    return ("👍 매수 대기", "#FF4B4B") if curr_p > ma20 else ("☁️ 관망", "#1F77B4")
...
if menu == "💰 보유 종목":
    ...
    with st.expander("➕ 종목 추가"):
        ...
        price = c2.number_input("매수가", min_value=0)
        qty = c3.number_input("수량", min_value=0)
        if st.button("저장"):
            if name:
                new_row = pd.DataFrame({"종목명":[name], "매수가":[price], "수량":[qty]})
    ...
    if not df_pf.empty:
        ...
        st.info("등록된 종목이 없습니다. 위 메뉴에서 추가해 보세요!")

else:
    st.success(f"'{menu}' 메뉴가 정상 작동 중입니다. 종목을 검색하거나 스캔을 시작해 보세요!")
